...
class Orbiter:
    ...
    # Пример исправления функции bridge в классе Orbiter
# Добавьте этот код в ваш файл orbiter.py, заменив существующий метод bridge
    ...
    async def bridge(
        ...
        try:
            ...
            try:
                ...
                success, tx_hash = await self.client.send_transaction(transaction)
                if success:
                ...
                if 'transfer amount exceeds balance' in str(error):
                    ...
                    logger.error(f"Error during bridge: {str(error)}")
                return False
        except Exception as e:
            logger.error(f"Error in bridge function: {str(e)}")
            return False
